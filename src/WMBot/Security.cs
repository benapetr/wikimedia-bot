            ...
            foreach (SystemUser u in Users) {
                if (u.Name == user) {
                    if (GetLevelOfRole (u.Role) > GetLevelOfRole (origin.Role)) {
                        ...
                        IRC.DeliverMessage (Localization.Localize ("RoleMismatch", this._Channel.Language), channel);
                        return;
                    }
                    if (u.Name == origin.Name) {
                        // users aren't permitted to delete themselve
                        IRC.DeliverMessage (Localization.Localize ("Trust2", this._Channel.Language), channel);
                        return;
                    }
                    Users.Remove (u);
                    Save ();
                    IRC.DeliverMessage (Localization.Localize ("Trust3", this._Channel.Language), channel);
                    return;
                }
            }
            IRC.DeliverMessage (Localization.Localize ("Trust4", this._Channel.Language), channel);
            return;
        }
